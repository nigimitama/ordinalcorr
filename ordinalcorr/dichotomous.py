...
def biserial(x: npt.ArrayLike, y: npt.ArrayLike) -> float:
    r"""
    Compute the biserial correlation coefficient between a continuous variable :math:`x`
    and a dichotomized variable :math:`y\in\{0,1\}`, assuming :math:`y` is generated by discretizing a latent continuous variable.
    ...
    x : array-like
        Continuous variable.
    y : array-like
        Dichotomous variable, assumed to be derived from a latent continuous variable.
    ...
    .. math::
        :label: biserial_correlation

        r_{b} := r_{pb} \frac{\sqrt{p (1 - p)}}{\phi(z)}

    where

    - :math:`r_{pb}` is the point-biserial correlation coefficient (equivalent to Pearson's correlation coefficient between :math:`X` and the dichotomous :math:`Y`)
    - :math:`p` is the proportion of observations where :math:`Y = 1`
    - :math:`\phi(z)` is the probability density function of the standard normal distribution
    - :math:`z` is the :math:`p`-th quantile (percentile point) of the standard normal distribution, i.e. :math:`\Phi(z) = p`.

    It is known that Pearson's product-moment correlation coefficient between :math:`X` and dichotomized :math:`Y`
    underestimates the true correlation (attenuation of correlation) [1]_ [2]_.
    Therefore, the biserial correlation coefficient corrects it by the inverse of the amount of attenuation :math:`\frac{\sqrt{p (1 - p)}}{\phi(z)}`.
    ...
    .. math::
        ...
        r_{b} := \frac{\bar{X}_1 - \bar{X}_0}{s_X} \frac{p (1 - p)}{\phi(z)}
    ...
    This is equivalent to the equation :eq:`biserial_correlation`.

    References
    ----------
    .. [1] Peters, C. C., & Van Voorhis, W. R. (1940). Statistical procedures and their mathematical bases. McGraw-Hill.
...
def point_biserial(x: npt.ArrayLike, y: npt.ArrayLike) -> float:
    r"""
    Compute the point-biserial correlation between a continuous variable :math:`x`
    and a dichotomous variable :math:`y\in\{0,1\}`, assuming :math:`y` is a true dichotomous variable
    (i.e. :math:`y` is not generated from a latent continuous variable).
